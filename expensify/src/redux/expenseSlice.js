...
export const fetchCurrencyRate = createAsyncThunk('', async (body, { dispatch }) => {
  try {
    ...
    if (baseCurrency !== newCurrency) {
      const res = await fetch(`https://api.frankfurter.dev/v1/latest?symbols=${newCurrency}&base=${baseCurrency}`);
      const data = await res.json();
      const exchangeRate = data.rates[newCurrency] || 1;
      return { exchangeRate, viewingCurrency: newCurrency };
    }
    return { exchangeRate: 1, viewingCurrency: newCurrency }
  } catch (e) {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchCurrencyRate.fulfilled, (state, action) => {
        const { exchangeRate, viewingCurrency } = action.payload || {};
        state.exchangeRate = exchangeRate;
        state.viewingCurrency = viewingCurrency;
